...
bot.command("update", async (ctx) => {
  ...
  await ctx.reply("<blockquote>⏳ Sedang mengecek update...</blockquote>", { parse_mode: "HTML" });

  try {
    const { data } = await axios.get(repoRaw);

    if (!data || data.length < 50) {
      return ctx.reply("<blockquote>❌ Update gagal: File kosong / tidak valid!</blockquote>", { parse_mode: "HTML" });
    }

    fs.writeFileSync("./index.js", data);

    await ctx.reply("<blockquote>✅ Update berhasil!\nSilakan restart di panel anda</blockquote>", { parse_mode: "HTML" });
    ...
    ctx.reply("<blockquote>❌ Update gagal.\nPastikan repo dan file tersedia.</blockquote>", { parse_mode: "HTML" });
  }
});
...
bot.command("XforceloseV1", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
  if (!q) return ctx.reply(`🪧 ☇ Format: /XforceloseV1 62×××`);
  let target = q.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
