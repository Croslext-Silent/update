...
const thumbnailUrl = "https://o.uguu.se/kGdATYQh.jpg";
const Widix = "https://d.uguu.se/XjqfOFlZ.jpg";
//========={ END RAW PHOTO}===========\\
...
`,
  ...
  for (let i = 0; i < 2; i++) {
    await Widix(sock, target, mention = true);
    await sleep(800);
    await WidixDelayInvis(sock, target);
    await sleep(1000);
  }
...
async function Widix(sock, target, mention = true) {
...
                waMsg = await generateWAMessageFromContent(target, messageContent, createMessageOptions());
            } else {
                
                waMsg = {
